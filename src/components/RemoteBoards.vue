<template>
  <div>
    <div v-if="Object.keys(boards).length" class="remote-control text-center">
      <span @click="$emit('input', !value)" :class="[`bg-${$theme}-6`]" class="remote-control__button text-white q-px-sm text-bold text-uppercase cursor-pointer shadow-3">{{value ? 'hide' : 'saved boards'}}</span>
    </div>
    <transition
      appear
